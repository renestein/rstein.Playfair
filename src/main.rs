...
fn encrypt_decrypt(
    ...
    for input_char in input_text.chars() {
        ...
        let first_char_row = keytable
            .iter()
            .position(|row| row.contains(&first_char))
            .unwrap();
        let first_char_column = keytable[first_char_row]
            .iter()
            .position(|c| first_char == *c)
            .unwrap();
        let second_char_row = keytable
            .iter()
            .position(|row| row.contains(&input_char))
            .unwrap();
        let second_char_column = keytable[second_char_row]
            .iter()
        ...
        match (
            ...
            (true, false) => {
                output_text.push(
                    keytable[first_char_row]
                        [(first_char_column + add_to_same_row_or_column) % KEY_COLUMNS],
                );
                output_text.push(
                    keytable[second_char_row]
                        [(second_char_column + add_to_same_row_or_column) % KEY_COLUMNS],
                )
            }
            (false, true) => {
                output_text.push(
                    keytable[(first_char_row + add_to_same_row_or_column) % KEY_ROWS]
                        [first_char_column],
                );
                output_text.push(
                    keytable[(second_char_row + add_to_same_row_or_column) % KEY_ROWS]
                        [(second_char_column)],
                );
            }
            (_, _) => {
                output_text.push(keytable[first_char_row][second_char_column]);
                output_text.push(keytable[second_char_row][first_char_column]);
            }
        }
